...
<адресная команда> ::= "LOAD" | "SAVE" | "PUSH" | "POP" | "ADD" | "SUB" | "CMP" | "JMP" | "JMN" | "JMNN" | "JMZ" | "JMNZ" | "JMC" | "JMNC"
...
Работа каждой инструкции выполняется в нескольких тактах:
 ...
 - Чтение операндов из памяти (если требуется):
   ...
   - Если читается относительный адрес, то выполняется еще одно чтение операнда.
 - Выполнение операции:
...
| JMNZ    | +        | +         | 3                 | Перейти в заданную ячейку если значение в аккумуляторе не равно нулю   |
